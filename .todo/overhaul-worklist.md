...
- **Batch 2b DONE 2026-06-11** (13 pages written; financial+reporting groups gate-reviewed, criticals fixed). ⚠️ CARRY-OVER for next session: (a) the config-pages gate review (work-schedule, custom-fields, roles-authorisations, assignments) was interrupted — re-run it before or with batch 2c; (b) non-critical 2b warnings not yet applied: "entity" prose in billing.mdx/costs.mdx; inert **Share** wording in reports.mdx/custom-reports.mdx FAQ+table; budgets.mdx task-budget mention + missing plan-gating Info + icon="settings"→"gear"; expenses.mdx currency-default + Amount/Quantity column labels; data-exports.mdx subtotal/period overstatements + card icons; **✕**→(**Remove link**) on both add-in pages. (c) NEW finding: GDPR/Export-your-data Settings entries are inert placeholders (catalog corrected; data-exports vapor section removed).
