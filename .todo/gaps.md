...
Result of the first pass: **0 Missing, 18 Partial** across 7 sections. **Verification pass:** every one of the 18 Partial entries was re-opened after `/write` and `/review`; the capability named in each `needs:` note is now present on the mapped page. **18 of 18 Resolved, 0 still open.** Two scope corrections surfaced by code checks during writing: the item-side **Who has access?** panel does not accept whole categories (only the person/tag **Show or hide** selectors and the **Manages** panel do), and custom-field and report-folder deletions are refused with a plain message without a link to the blocker — pages, catalog, and news were aligned accordingly. One out-of-scope fix: recurring-task rows on `staffing.mdx` (hidden behind `SHOW_TASK_RECURRENCE = false`) were removed per the feedback rule on production-suppressed controls.
