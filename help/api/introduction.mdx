...
<Steps>
  <Step title="Open your user menu">
    In Beebole, click the button with your initials at the bottom of the left sidebar.
  </Step>
  <Step title="Open the API Key panel">
    Click **API Key**. Beebole creates your key automatically the first time you open this panel.
  </Step>
  <Step title="Copy your API key">
    Click **Copy** to copy the full key to the clipboard. Click **Reset** to revoke the current key and generate a new one.
  </Step>
...
The panel shows a single active API key, and it does not expire. The key authenticates as the person it belongs to, so requests inherit that person's role and permissions. To read time records, people, or projects, the key's owner needs matching **View** permissions on **Timesheet entries**, **People**, and **Reports**. See [Roles & permissions](/help/documentation/roles-authorisations) for the full list. If a request returns a `permissionsErrors` entry, the key's owner is missing the permission for that field. An administrator has to grant it, or you have to issue the key from a person whose role already includes it.
