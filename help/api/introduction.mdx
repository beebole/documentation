---
title: "Introduction, Endpoint & Authentication — Beebole API"
...
Each account has a single API key, and it does not expire. The key authenticates as the person it belongs to, so requests inherit that person's role and permissions.
...
Beebole resolves permissions per operation. If a request asks for data or an action the linked person is not authorized for, the response omits that data or returns an authorization error for the affected field rather than failing the whole request.
