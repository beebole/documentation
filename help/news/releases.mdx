...
<Update label="August 2026" tags={["Reports","Budgets","Planning","Timesheet","Integrations","Settings","AI"]}>
  ...
  - Tasks can now carry start and end hours, not just dates: uncheck **All day** in a task's date panel to set the exact times, pre-filled from the owner's working hours. Dragging, resizing, splitting, and copying keep the hours, and typing **Planned hours** reflows the task to the exact minute the owner's calendar can hold — skipping breaks, days off, and time off, and rolling to the next working day only when the current one runs out.
