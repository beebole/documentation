...
<Update label="September 2026" tags={["Planning","Timesheet","Reports","Settings","Integrations","AI"]}>
  ...
  - The **Potential owners** column in the Gantt has separate buttons to add a person or a tag, and you can remove a person, tag, or project straight from a task row by hovering its badge and clicking the X, without opening the edit popup.
  ...
  - Creating a booking on the **Unassigned** row of [Staffing](/help/documentation/staffing) now behaves like any other row: nothing is saved until you pick the person or task, so an editor you abandon leaves no stray booking behind.
  - A project row above the lowest level stands out in the accent color in [Staffing](/help/documentation/staffing), with a note explaining that its bookings belong further down the hierarchy.
  ...
  - [Suggestions](/help/documentation/ai) reach the [mobile timesheet](/help/documentation/mobile): they appear as ghost rows inside each day, next to your real entries. Accept one with a single tap, or open it to change the duration, the start and end times, point it at another task or working activity, start a timer on it, or dismiss it.
  - Emptying either the start or the end time of a suggestion turns it back into a duration-only draft. And a suggestion that sits on a staffing booking, or on a task your settings don't let you log to, now always asks you to pick a project first and tells you why the accept button took you to the picker.
  ...
  - An [approval](/help/documentation/approval) request for time logged on a subproject now also reaches the managers of its parent projects, including when an existing request is re-routed.
  ...
  - The **Master data review** screen is now available to every administrator from the Settings menu: pick people, projects, tasks, tags, or any other configuration item, build columns by walking into its fields and relations — custom field values included — filter, download, and describe bulk changes in plain language with a before/after preview and undo. Updates can now also add text around existing values ("prefix their external ID with ext-"). The screen now has explicit **Read your data** and **Change your data** tabs, and a change can target named rows only ("set the external ID of Anna to abc") or fill a field with a running counter ("ext-3, then ext-4, …"), optionally only where the field is still empty. See [Master data](/help/documentation/master-data).
  - Managers and [assignments](/help/documentation/assignments) can now cover a whole project, task, or tag category instead of picking items one by one: one click on the category row selects it, a manager of a category manages everything inside it (including anything added later), and the same applies when assigning projects, secondary projects, and tasks to people and to tags. Every category now has a color of its own, assigned automatically and changeable from the category menu, shown in selectors and on filter chips.
  ...
  - The first column of a [Master data](/help/documentation/master-data) table is now a normal column header: filter on it, and switch to another list (people, projects, tags…) or another category straight from its menu. Tables also show only the lowest level of a hierarchy by default, so parent rows no longer pad your export — pick a specific level from the scope menu when you want one.
  - Asking for your data in plain English now works on the tag and project lists too, and a person's name in your question is read as a filter: ask "who does Anna manage?" and the matching column is added to the table.
  - [Master data](/help/documentation/master-data) reviews and the report builders now share one sentence box that keeps the whole conversation: each question and its answer stay on screen, with earlier turns available behind **Whole conversation**. Reading your data and changing it are two tabs on that box, with **Apply** and **Cancel** next to the sentence and the undo of your last change always in reach.
  - Project, task, and tag badges spell out the full parent path whenever it fits the space available, falling back to initials only when the label would be clipped, and their tooltips always show the complete path.
  ...
  - Time exported to [QuickBooks](/help/integrations/quickbooks) now carries whole hours and minutes instead of a decimal hour value, so entries land on the exact duration tracked. A second export is refused with a clear message while one is already running, so a double-click or a second tab can no longer export the same entries twice.
  - Connections to [QuickBooks](/help/integrations/quickbooks), [Xero](/help/integrations/xero), [Asana](/help/integrations/asana), and [Jira](/help/integrations/jira) no longer drop out and need reconnecting when several requests refresh the connection at once.
  - The [Excel](/help/documentation/excel-addin) and [Google Sheets](/help/documentation/gsheets-addon) add-ins fill in the **Activity** column again instead of leaving it blank.
  - The [browser extension](/help/documentation/browser-extension) no longer reads page content at all: only the site address, the page title, and the duration leave your browser. And if your API key is rejected, it stops tracking and says so instead of retrying silently.

  {/* news-cursor: 2026-09-17 */}
</Update>
