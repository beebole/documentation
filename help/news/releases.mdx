...
<Update label="August 2026" tags={["Reports","Planning","Timesheet","Settings","AI"]}>
  August adds three new reports — **Revenue at Risk**, **Utilization**, and **Timesheet Compliance** — teaches your timesheet to assemble and submit itself from suggestions, makes report folders personal and shareable, and sharpens the Gantt and Staffing views for day-to-day resource planning.
  ...
  - [Report folders](/help/documentation/reports) are now personal: you see your own folders plus any shared with you, and you can **Share** a folder with specific people or with everyone under a tag. Shared folders are view-only for recipients — the owner keeps full control.
  ...
  - Planned vs. Real is now a browsable list: pick a person or project level as the axis, step through entities with the arrows or keyboard, and filter with chips that stay in the URL so a filtered view can be shared — the time window and granularity are worked out automatically.
  ...
  - [Budgets](/help/documentation/budgets) now carry a start date, so a project can hold several budgets over time — each applies from its own date onward.
  - A budget's quantity can be stated in **days** instead of hours, measured against each person's schedule, and every budget can hold a free-text note (purchase order, customer agreement, …) shown on its card.
