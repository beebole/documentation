...
<Update label="September 2026" tags={["Planning","Timesheet","Reports","Settings","AI"]}>
  ...
  - Time can no longer be logged on a project that has sub-projects: parent projects aggregate their children and only the lowest level accepts entries, the same rule that already applied to parent tasks.
  ...
  - Alerts that fire in bulk — a mass timesheet submit, an import, an automated catch-up — now arrive as a single summary email or push instead of one message per event. Summary emails list the first few items per category and close with a count of the rest.
