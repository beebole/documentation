...
<Update label="September 2026" tags={["Planning","Timesheet","Reports","Settings","Integrations","AI"]}>
  ...
  - When a delete is refused because something still uses the item — a time-off or expense type with allowances or records, a report folder with reports, a custom field with values, or a person or project referenced by time settings, task status, schedules, managers, or task owners — the message now names the blocking item and links straight to the panel where you can remove the reference.
