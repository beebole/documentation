...
keywords: ["beebole", "migration guide", "new beebole platform", "free trial", "what's changed", "legacy account", "which version of beebole", "dcaa", "sox"]
...
<Warning>
  Data from the Legacy system can only be migrated into the new Beebole **as is**. You choose _which_ data to migrate, but not to change its configuration on the way in. For example, migrated projects keep the same _client \> project \> subproject_ hierarchy they had in the Legacy system. If you want a different structure in the new platform, we recommend exporting and saving your historical data and starting fresh. See [Review your structure before you switch](#review-your-structure-before-you-switch).
</Warning>
...
## Which Beebole am I using?

The two platforms look different, and the pages in the other tabs of this site only describe the new one. Before following an instruction, check which app you are signed in to:

| | New Beebole | Legacy Beebole |
| :-- | :-- | :-- |
| **Address** | app.beebole.com | beebole-apps.com, or the login button on beebole.com |
| **Layout** | A left sidebar with **Timesheet**, **Planning**, **Reports**, **People**, and **Projects**, and a button with your initials at the bottom | A home screen and a Settings screen made of *modules* — Timesheet, Account, Timesheet Settings, API Token, and so on |
| **Sign-in** | Passwordless: a one-time email code, a passkey, Google or Microsoft, or SSO | Email and password |
| **Settings** | The button with your initials opens **Account Settings**, **Person Roles**, **Work Schedules**, and the other settings screens | "Settings" in the top right corner opens the Settings screen |
| **Roles** | Custom roles under **Person Roles** | Fixed user groups: administrator, employee, contractor, project manager, team leader |
| **API** | The [GraphQL API](/help/api/introduction) at `app.beebole.com/graphql`, with the key from **Your API key** | The [Legacy API](/help/legacy/api), with the token from the *API Token* module |

If the Legacy column is what you see, use the [Legacy documentation](/help/legacy/index) for now: the current pages describe controls your app does not have, such as **Person Roles** or the initials button. Screenshots pasted into the docs assistant from the Legacy app lead to the same dead end.
...
| Topic | What's new |
| :-- | :-- |
| **Audit trail** | Every change is recorded in the [Audit Trail](/help/documentation/audit-trail) without a setting to switch on, unlike the Legacy checkbox. |
| **DCAA ownership and SOX reports** | The Legacy *Timesheet Settings* toggles no longer exist under those names. The controls they stood for are the **Only the owner or an admin can edit entries** restriction in [Timesheet and Planning Settings](/help/documentation/timesheetSettings#restrictions), which keeps managers from changing other people's hours; the [approval history](/help/documentation/approval#approval-history) kept on every timesheet; the [Lock date](/help/documentation/timesheetSettings#lock-date) that freezes closed periods; and the [Timesheet Compliance](/help/documentation/reports#timesheet-compliance) report, which shows submissions period by period. If your auditor needs a specific extract, [support@beebole.com](mailto:support@beebole.com) can help you build the report. |
...
<AccordionGroup>
  ...
  <Accordion title="How do I know whether I am on the new Beebole or on Legacy?">
    Look at the address bar and the layout. The new Beebole runs at app.beebole.com, has a left sidebar, and signs you in without a password. Legacy runs at beebole-apps.com, is organized in modules on a home screen and a Settings screen, and uses a password. See [Which Beebole am I using?](#which-beebole-am-i-using) for the full comparison.
  </Accordion>

  <Accordion title="Does the new Beebole have DCAA ownership and SOX reports like Legacy?">
    Not as named toggles. The new platform records every change in the audit trail without a setting, keeps the approval history of each timesheet, and lets you stop managers from editing other people's hours with the **Only the owner or an admin can edit entries** restriction — the rule Legacy called DCAA ownership. Submission compliance per period is in the Timesheet Compliance report. Whether that covers your audit requirements is for your compliance team to confirm; support can walk you through the controls.
  </Accordion>
</AccordionGroup>