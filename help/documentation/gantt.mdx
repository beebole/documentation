...
Beebole's Gantt chart displays your [tasks](/help/documentation/planning) as bars on a timeline, so you can see start and end dates, sequence work with dependencies, and spot scheduling overload at a glance. It is one of the three views of the **Planning** page, alongside the [Kanban board](/help/documentation/kanban) and the [Staffing view](/help/documentation/staffing) — all show the same tasks, so a change in one view appears instantly in the others.
...
- A fixed **week**, **month**, **quarter**, or **year** — the chart fits that window to your screen and lists only the tasks that fall inside it (parents and recurring series are kept).
...
- **Work on several tasks at once** — Select several bars, then drag them together to reschedule the whole group in one move, hold **⌘** (Alt on Windows/Linux) while dragging to drop dated copies, or grab any selected bar's edge to shift every selected bar's dates by the same amount. Each batch is a single undo step.
...
Dependencies sequence tasks: a dependent task is expected to follow the tasks it depends on. Dependency links are drawn between the bars on the timeline, color-matched to the tasks at each end, and they follow the bars live while you drag. Dependencies are managed in the **Dependencies** column — add it from the **Columns** menu if your view doesn't show it.
...
The **Dependent tasks movement options** setting — in the **⋯** menu on the active view tab — controls what happens to dependent tasks when you move a task they depend on:
...
The setting applies to the whole planning.
...
When the chart is grouped — for example by **Owner** — each group header row displays workload bars: one bar per period (day or week, following the current **Period** granularity). Without grouping, a single summary row shows the same information for everyone.
...
The tabs above the task list are saved views. Each Gantt view stores its own **Period**, **Columns**, and **Group by** settings, plus its filters.
...
<AccordionGroup>
  <Accordion title="Does Beebole's Gantt chart have a month view?">
    No. The Gantt timeline runs **Infinite by day** or **Infinite by week**, selected under **Period** in the view tab's menu — the week granularity is the one to use for a long-range overview. The same menu can also lock the view to a fixed week, month, quarter, or year fitted to your screen.
  </Accordion>
  ...
  <Accordion title="Can I have several Gantt views with different columns?">
    Yes. Click **Add a view** and select **Gantt chart**. Each saved view in Beebole keeps its own column selection, grouping, scale, and filters, so you can switch between setups without reconfiguring anything.
  </Accordion>
