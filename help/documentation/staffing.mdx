---
title: "Staffing view: plan who works on what, and when"
...
**Lowest level** keeps only the projects that have nothing below them, so you never see a parent row duplicating the work of its children. When a level you picked shows a project that still has sub-projects, its row stands out in the accent color with the note **Not at the lowest level: move its bookings to one of its sub-projects** — bookings belong on the projects at the bottom of the hierarchy, where time is logged. With a project grouping, an owned booking that has no row to sit on — the task is linked to no project of that category, or only to a parent — falls into the row at the top of the view, labeled **No** followed by the grouping name.
...
<Steps>
  ...
  <Step title="Pick the person and the project">
    The editor shows both sides of the booking: the person (**Select the owner**) and the project. The row you dragged in fills one side for you — a person's row prefills the person, a project row prefills the project — and the booking saves as soon as you pick the other, with no confirmation step. The editor then stays open on the new booking and every further change applies immediately: remove the person or project badge to pick another one, whatever the rows are grouped by.
  </Step>
...
- **Move** — Drag a bar left or right to shift its dates. The bar moves by whole days — or by whole weeks when the timeline is set to **Infinite by week** — so it keeps its exact length and the weekday it starts on instead of snapping to the grid; other bookings restack live as you drag.
...
- **Reassign** — Click a bar to open its editor and change the person, the project, or the allocation. Both the person and the project can be changed on any booking, whether the rows are people or projects. Changes are saved automatically.
...
A task that has subtasks has a schedule the Staffing view must not change: its dates come from its subtasks. When you try to edit one, Beebole shows **Managed in the Gantt** with the message "This task has subtasks, so its schedule is managed in the Gantt."
...
Click the pin icon on the row to keep it under the timeline header while you scroll — its tooltip reads **Keep this row under the header** — so its bars stay within reach when the person you want to drop them on sits further down the list. Click it again (**Stop keeping this row under the header**) to let the row scroll with the others. Like the collapsed state, the pin is remembered per view.

You can also draw a booking straight on the row. Nothing is saved until you pick the person or the task it is for, so an editor you open and then abandon leaves no stray booking behind — the same as on any other row.
...
Beebole looks back to the last period that actually holds bookings, not just the one immediately before, and fills every empty period between it and the one you are looking at — so a run of unstaffed weeks is refilled in a single action. Anything that would spill outside the period is clipped. If there is nothing to copy, Beebole says so: **No earlier bookings to copy**.
...
The Staffing view has a keyboard cursor that moves from bar to bar, so you can review and open bookings without the mouse:

- **Arrow Down** / **Arrow Up** — Move to the closest bar in the lane below or above within the same row, then on to the next or previous row. Rows without bookings, and the **Unassigned** row while it is collapsed, are skipped.
- **Arrow Right** / **Arrow Left** — Move to the next or previous bar in the same lane; when the lane has none, to the nearest bar that lies entirely after (or before) the current one in any lane of the row.
- **Enter** — Open the task panel for the bar under the cursor; press it again on the same bar to close it. While the panel is open, it follows the cursor as you move.
- **Shift+Arrow** — Select the bars in the lane between the cursor and the bar you land on, like **Shift+Click**.
- **⌘+Arrow** (Ctrl on Windows and Linux) — Add the bar you land on to the selection, or remove it, like **⌘+Click**.

With no bar selected yet, the first arrow key lands on the first row's bar closest to today, and the timeline scrolls to keep the cursor in sight. A selection built with the keyboard behaves like any other: drag it, ⌘-drag it, or press **Delete** to act on every selected booking at once. The Gantt and List views add tree keys for expanding, collapsing, and indenting tasks — see [Keyboard navigation in the Gantt chart](/help/documentation/gantt#keyboard-navigation).

<Info>
  **Enter** is left to any field that has focus. If the keys do not respond, click an empty spot on the timeline first.
</Info>
...
<CardGroup cols={2}>
  ...
  <Card title="Gantt chart" icon="chart-gantt" href="/help/documentation/gantt">
    Schedule tasks on a timeline, sequence them with dependencies, and edit every task field in place.
  </Card>
...
<AccordionGroup>
  ...
  <Accordion title="Why can't I move a booking in the Staffing view?">
    The task's schedule is managed in the Gantt. Tasks with subtasks show a **Managed in the Gantt** message — open the Gantt chart to change their dates. Bookings with dependencies can be moved and resized directly in Staffing; their linked bookings follow along.
  </Accordion>
  ...
  <Accordion title="Can I make a booking or task repeat every week?">
    Not yet. Beebole has no recurring bookings or tasks: each booking is a single task between two dates. To carry a weekly pattern forward, lock the view to a period and use **Copy the previous period**, which refills empty periods from the last one that holds bookings, or ⌘-drag a selection of bars to drop dated copies.
  </Accordion>
</AccordionGroup>
