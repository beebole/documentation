...
<Info>
  Beebole AI runs on Beebole's own AI models, self-hosted on Beebole-operated servers in the same region as your data. Those servers have no public address — only Beebole's backend reaches them, behind a firewall — and nothing is sent to a third-party AI provider.
</Info>
...
<AccordionGroup>
  <Accordion title="Does using Beebole AI send my data to OpenAI, Anthropic, or another AI provider?">
    No. Beebole runs its own AI models, self-hosted on Beebole-operated servers in the same region as your data. Those servers are firewalled and have no public address, so only Beebole's backend can reach them, and nothing is sent to a third-party AI provider.
