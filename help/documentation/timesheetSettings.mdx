...
<Info>
  No button with your initials at the bottom of the sidebar? Then you are signed in to the Legacy platform (beebole-apps.com), where these settings live in the *Timesheet Settings* module of the Settings screen — see the [legacy settings page](/help/legacy/settings#timesheet-settings-module). [Which Beebole am I using?](/help/guides/migration#which-beebole-am-i-using) explains how to tell the two apart.
</Info>
...
| **Hide the non-billable option** | Removes the **Non-billable** checkbox from the entry details, so nobody can mark a time entry as non-billable. |
...
**Hide the non-billable option** also protects the data behind the form. Any attempt to mark an entry as non-billable — from the timesheet or through the API — is refused with **Your timesheet settings do not offer the non-billable option.** Entries marked before you activated the restriction keep their flag, and clearing it stays allowed. When entries are copied to another period, the copies arrive without the non-billable flag, even if the originals carried it. The setting follows the timesheet owner: like every restriction, it is resolved through the account, the person's tags, and the person, so a rule set on a tag applies to everyone under it.
...
Two more rules apply to every time entry without appearing in the **Restrictions** list, because they are configured on the items they protect:
...
<AccordionGroup>
  ...
  <Accordion title="Can I lock time entries for a single project?">
    Not with the **Lock date** — it freezes a person's whole timesheet up to a day, account-wide or for one tag or person, never per project. To stop time being added to one project after a date, open that project and set the **To** date of its **Valid period for time entry**: Beebole refuses entries on that project after that day while other projects stay open. To close a whole week for everyone, set the account-wide **Lock date** to the week's last day once approvals are done.
  </Accordion>

  <Accordion title="Who can set the lock date — project managers or only admins?">
    Administrators always can. Otherwise it takes a role with **Edit** access on the **Timesheet and planning settings** permission in Beebole, which then covers the people its targets reach — give a role **Edit: Managed people** there and its holders can set the lock date on their own team's profiles. None of the built-in roles other than **Admin** include it, since the lock is a timesheet setting rather than a project or team setting.
  </Accordion>
</AccordionGroup>