...
<Steps>
  ...
  <Step title="Set when it starts (optional)">
    By default a budget counts **From the start** — everything ever logged on the project. Set a date in **From** to have the budget apply from that day onward instead.
  </Step>
</Steps>
...
A project can hold more than one budget: click **Add another budget** to create the next one. Each budget applies from its own **From** date onward, so successive budgets model renewals, yearly envelopes, or a re-negotiated scope — the list shows them in chronological order, and **Back to all budgets** returns to the list.
