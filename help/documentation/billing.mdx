...
The **From** date is evaluated at each level independently. A project rate only applies to entries dated on or after its own **From** date — earlier entries fall back to the next level in the cascade, even when a project rate exists. If an organization rate appears to override a more specific project or person rate, check that the specific rate's **From** date is on or before the time entry's date.
...
<AccordionGroup>
  ...
  <Accordion title="Why is my organization rate overriding my project or person rate?">
    Beebole always prefers a more specific rate — project first, then person — over an organization or tag rate. If an organization rate appears to win instead, the specific rate's **From** date is likely later than the time entry's date, so the specific rate does not apply to that entry and the lookup continues up to the organization level. Add a project or person rate with a **From** date on or before the entry's date to make it apply.
  </Accordion>
</AccordionGroup>