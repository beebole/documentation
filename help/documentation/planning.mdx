...
keywords: ["beebole", "tasks", "task management", "task planning", "plannings", "task statuses", "subtasks", "task categories"]
...
<Info>
  To work with tasks, click **Planning** in the sidebar. The same tasks can be displayed in three views: the [Gantt chart](/help/documentation/gantt) for timelines, the [Kanban board](/help/documentation/kanban) for status columns, and the [Staffing view](/help/documentation/staffing) for planning by person.
</Info>
...
Every task belongs to a planning — a separate board with its own statuses and its own level names for the hierarchy. New Beebole accounts start with one planning named **Main planning**, with the statuses **Backlog**, **Queue**, **In progress**, and **Done**.
...
- **Custom fields** — Organization-defined fields (text, numbers, dates, URLs, booleans) that appear in the task detail panel. Define them in **Settings** \> **Custom Field** and make them visible for tasks — visibility can even be limited to specific plannings. See [Custom fields](/help/documentation/custom-fields).
