...
- **Owner** — The single person doing the task. Pick them in the task detail panel (**Select the owner**); a task can only have one owner. Next to the owner you can set **% FTE**, the share of their working time dedicated to the task. Beebole uses the owner's [work schedule](/help/documentation/work-schedule) and this percentage when fitting planned time between the task's dates. The task detail panel only takes the allocation as **% FTE**; to enter it as **h/day** or a **Total** number of planned hours instead, open the task's bar on the [Staffing timeline](/help/documentation/staffing#creating-a-booking) and switch the unit there — it is the same underlying task and updates everywhere.
