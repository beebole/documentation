...
<AccordionGroup>
  ...
  <Accordion title="Can I reject only some entries on a person's timesheet?">
    No. **Approve** and **Reject** always act on the whole submitted period, not on individual rows or entries. If only a few entries need fixing, the better path is to correct them yourself using the **Edit timesheet** button on the person's row (see [Editing a submitted or approved timesheet](#editing-a-submitted-or-approved-timesheet)) and then approve, or reject the timesheet with a comment listing what the owner should change before resubmitting.
  </Accordion>
