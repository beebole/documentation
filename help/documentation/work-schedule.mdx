...
<AccordionGroup>
  ...
  <Accordion title="Do work schedules automatically fill in timesheets?">
    No. Work schedules define the hours each person is *expected* to work, but they don't create timesheet entries. Beebole shows the scheduled hours in each day header, hatches non-working days, and compares scheduled to logged time in the totals row — but each person still enters their own time on the timesheet. To require scheduled time to be filled before a period can be submitted, use the **Require filling the whole schedule to submit** restriction in [Timesheet and Planning Settings](/help/documentation/timesheetSettings).
  </Accordion>
</AccordionGroup>
