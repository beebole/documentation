...
To analyze spending, click **Reports** in the sidebar and choose **Expenses** as the report's source. You can group expense records by expense type, person, project, tag, or period, and filter them like any other report. Expense columns include **Expense amount**, **Expense quantity**, and **Expense billing** — the billed amount with the category's markup applied. See [Reports](/help/documentation/reports) for folders, charts, sharing, and exports.
...
<AccordionGroup>
  ...
  <Accordion title="What currency is an expense recorded in?">
    Each expense record carries its own currency, which you can change on the **Amount** field. The default comes from the person's currency settings, or from the organization's currency when the record is linked only to a project.
  </Accordion>
</AccordionGroup>
