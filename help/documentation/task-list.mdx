...
- Anything that is a linked item — the owner, the potential owners, the tags, the dependencies, the projects of each category — is drawn as a badge with its ancestors, not as bare text. Click a badge to open it, or **Shift**+click it to add it as a filter.
...
To search by name, use the task filter's **contains** or **starts with** condition. **Shift**+clicking any badge in the table adds that item as a filter, which is the fastest way to narrow down to one client, one owner, or one tag.
