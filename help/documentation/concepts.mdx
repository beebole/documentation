...
Projects, tasks, and tags appear throughout Beebole as badges — in lists, on Gantt and Kanban rows, in relation and tag panels, and on task bar tooltips. A badge spells out the full parent path of what it names whenever the space available can hold it, so **Acme Corp > Website Redesign** reads as itself rather than as an abbreviation. Only when the label would be clipped does the badge fall back to the initials of the parent levels, keeping the record's own name readable. Relation and tag lists give badges more room than tighter surfaces do, so paths stay spelled out there more often.
