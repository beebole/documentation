...
- **Tasks are independent planning items, not parts of a project.** Where projects model what hours go _into_, tasks model the work _to be done_. A task can optionally be linked to a project — which connects your planning and time tracking — but it exists on its own, with its own dates, owner, and status, managed in the Gantt chart and Kanban board.
...
<Info>
  Think of projects as _what_ your team tracks time against, tasks as _what you plan and schedule_, people as _who_ does the work, and tags as _how you organize_ everything for reporting and management.
</Info>
...
Beebole refuses to delete an item that something else still depends on. Instead of deleting it, Beebole shows a notification that explains what blocks the deletion and, in most cases, names the blocking item. Click the name to jump straight to the panel where the reference lives, remove it there, and try the deletion again.

| You try to delete | Blocked when | Clicking the name opens |
| --- | --- | --- |
| A project, task, person, or time off type | Time records are logged against it | The person's timesheet, on the date of one of those records |
| A time off type | Time-off allowances still use it | The **Absence allowances** panel of the record that holds the allowance |
| An expense type | Expense records use it | The **Expenses** panel of the person who logged them |
| A project | A task is linked to it | The task's **Projects** panel |
| A person | They own tasks | The task's **Owner** panel |
| A person | They manage other people | The panel of a person they manage, where the manager is set |
| A person | Approval events are recorded for them | The person's panel |
| A work schedule | It is assigned to a person, tag, or the organization | That record's **Work schedule** panel |
| A project or task category | It is selected in timesheet and planning settings | The **Timesheet and Planning Settings** panel of the person, tag, project, or organization using it |
| A project or task category | It still contains items | One of those items |
| A task status | Tasks still use it | The task's **Task details** panel |

Some refusals show a message only, because the fix is in the item itself rather than somewhere else in the account. A custom field that already holds values shows **This custom field cannot be deleted because values have been entered for it. Archive it instead.** A report folder that still contains reports shows **This folder cannot be deleted because it still contains reports. Move or delete them first.** The same applies to a tag that is still assigned, a role that is still assigned to people, a project that still has subprojects, and the last remaining task category or status.

<Info>
  When an item has history you want to keep, archive it instead of deleting it. Archived items disappear from the active lists, but every record and reference that points to them stays intact.
</Info>
...
When you open a person, project, tag, or task, the detail panel lists its attributes — billing rates, work schedule, custom fields, and so on. The URL follows the attribute you are reading: as you scroll, the address updates to the attribute whose header sits at the top of the panel, and clicking inside an attribute selects it too. Copy the URL at any moment and the link opens the same record with that attribute at the top of the panel.

- **Switch records without losing your place.** When you select another person, project, tag, or task while an attribute is in the URL, the panel opens the same attribute and scrolls it into view, so you can compare one setting across several records.
- **Links stay put while the panel loads.** An attribute opened from a link is held at the top of the panel while the rest of the content finishes loading, so sections that load later cannot push it out of view. It moves only once you scroll, click, or type inside the panel.
- **Collapsing an attribute clears it from the URL.** The link then opens the record without a specific attribute selected.
...
### Badges and parent paths

Projects, tasks, and tags appear throughout Beebole as badges — in lists, on Gantt and Kanban rows, in relation and tag panels, and on task bar tooltips. A badge spells out the full parent path of what it names whenever the space available can hold it, so a subproject reads as _Acme Corp > Website Redesign_ rather than as an abbreviation. Only when the label would be clipped does the badge fall back to the initials of the parent levels, keeping the record's own name readable. Relation and tag lists give badges more room than tighter surfaces do, so paths stay spelled out there more often.

Whatever the badge shows, hovering it gives you the complete path in a tooltip.
