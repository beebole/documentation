...
## Importing time entries from a file

Beebole has no import of time entries from a CSV file or a spreadsheet. Pasting rows from a spreadsheet exists for adding people, projects, tags, and tasks in bulk, not for hours. To bring in time that was recorded elsewhere, use one of these routes:
...
- **The API** — create entries for any number of people with `addTimeRecord`, from a script that reads your spreadsheet; see [Mutations](/help/api/mutations#time-records) and the [worked example](/help/api/examples/example-2).
...
<AccordionGroup>
  ...
  <Accordion title="Can I import time entries from a file or spreadsheet?">
    No. Beebole has no CSV or spreadsheet import for time entries — only people, projects, tags, and tasks can be pasted in bulk. To bring time in, copy a period, import calendar events, create entries through the API with `addTimeRecord`, or ask support for a legacy migration. See [Importing time entries from a file](#importing-time-entries-from-a-file).
  </Accordion>
