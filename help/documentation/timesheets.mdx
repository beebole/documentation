...
<AccordionGroup>
  ...
  <Accordion title="Can I unsubmit or recall my timesheet after clicking Submit?">
    Not on your own. Once submitted, the timesheet is locked and waiting in the [approval workflow](/help/documentation/approval). To reopen it for editing, ask a current-stage approver or an administrator to **Reject** it. Rejection returns the timesheet to you as **Rejected** so you can correct the entries and click **Resubmit**. An administrator can also reject an already-approved period to send it back.
  </Accordion>
  <Accordion title="Can I bulk import time entries from a spreadsheet or CSV?">
    The timesheet has no CSV upload for time entries. The paste-from-spreadsheet flows in Beebole cover master data — people, projects, tasks, tags — not the hours logged against them. To load time entries in bulk, either [import calendar events](#importing-calendar-events) from a connected Google or Microsoft calendar, or create records programmatically with the [GraphQL API](/help/api/introduction) (see [API examples](/help/api/examples/example-2)).
  </Accordion>
