...
<AccordionGroup>
  ...
  <Accordion title="Why is there no day, week, or month toggle on my timesheet?">
    The timesheet has no view switcher for the period. The **Timesheet period** (daily, weekly, bi-weekly, semi-monthly, or monthly) is a setting your administrator controls in **Account Settings** > **Timesheet and Planning Settings**, on the **Period & submission** tab. It can also be overridden per tag or per person, which can make your period differ from a colleague's. The only view toggle on the timesheet page itself switches between **Grid view** and **Calendar view**. See [Timesheet and Planning Settings](/help/documentation/timesheetSettings#timesheet-period).
  </Accordion>
  <Accordion title="Why does Add a row show no projects, only time off?">
    Two settings decide which projects appear in the **Add a row** list, and both are managed by your administrator:

    - **Project categories** — the timesheet only offers the project categories selected on the **Categories** tab of [Timesheet and Planning Settings](/help/documentation/timesheetSettings#categories). If none is selected, no project section appears.
    - **Assignments** — a project only appears if it is available to you. Projects can be hidden by default and granted per person or tag through the **Who has access?** and **Show or Hide** panels. See [Assignments](/help/documentation/assignments).

    If only a **Time Off** section shows up, ask your administrator to check both settings for your account.
  </Accordion>
