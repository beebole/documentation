...
Only one timer runs at a time, wherever you start it from. Starting a timer on another row, entry, or favorite pauses the current one and saves its time first.
...
<AccordionGroup>
  ...
  <Accordion title="Can I import time entries from a file or spreadsheet?">
    No. Beebole has no CSV or file upload for time entries. Time is entered directly in the timesheet grid or calendar view, with the timer, or by [importing calendar events](#importing-calendar-events) from Google or Microsoft. Pasting from a spreadsheet works for adding [people](/help/documentation/people), [projects](/help/documentation/projects), and [tags](/help/documentation/tags) in bulk, not time entries. To create time entries programmatically, use the `addTimeRecord` mutation in the [GraphQL API](/help/api/mutations). To bring time records over from an older Beebole account, use the [Legacy Migration](/help/documentation/legacy-migration) tool.
  </Accordion>
