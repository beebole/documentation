...
- **Columns** are the days of the current period. Today's date is highlighted, and each day's header shows the time reported so far against your scheduled hours. A day marked as a [public holiday](/help/documentation/public-holidays) is highlighted with a hatched pattern, and its name appears when you hover the header. Whether you have to log time on that day depends on the account's [timesheet settings](/help/documentation/timesheetSettings).
...
- **Create an entry** — Click and drag across an empty time slot. A popup opens to pick the project, task, or absence type; the start and end times come from where you drew.
