...
The board has a keyboard cursor that moves from card to card:

- **Arrow Down** / **Arrow Up** — Move to the next or previous card in the column.
- **Arrow Right** / **Arrow Left** — Move to the neighboring column, landing on the card at the same height (or on its last card when that column is shorter). Empty columns are skipped.
- **Enter** — Open the task panel for the card under the cursor; press it again on the same card to close it. While the panel is open, it follows the cursor as you move.
- **Shift+Arrow** — Select the cards in the column between the cursor and the card you land on, like **Shift+Click**.
- **⌘+Arrow** (Ctrl on Windows and Linux) — Add the card you land on to the selection, or remove it, like **⌘+Click**.

With no card selected yet, the first arrow key lands on the first card of the first column, and the board scrolls to keep the cursor in sight. A selection built with the keyboard behaves like any other: drag a selected card to move them all, or press **Esc** to clear it. The Gantt and List views add tree keys for expanding, collapsing, and indenting tasks — see [Keyboard navigation in the Gantt chart](/help/documentation/gantt#keyboard-navigation).

<Info>
  **Enter** is left to any field that has focus. If the keys do not respond, click an empty spot on the board first.
</Info>
