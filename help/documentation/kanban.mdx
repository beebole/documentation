...
- A **⋯** menu with **Archive** (archive every task in the column), **Unarchive** (restore the column's archived tasks), **Move left** / **Move right** (reorder the column), and **Delete** (only available when the column holds no tasks — including archived ones — and isn't the planning's only status).
...
<Tip>
  Keep the workflow short. New Beebole accounts start with one planning, **Main planning**, and four statuses — **Backlog**, **Queue**, **In progress**, **Done** — which is enough for most teams.
</Tip>
...
<AccordionGroup>
  <Accordion title="Can I use the Kanban board without setting up statuses first?">
    Yes. New Beebole accounts come with the default **Main planning** and the statuses **Backlog**, **Queue**, **In progress**, and **Done**, so the board works immediately. You can rename, recolor, reorder, add, or remove statuses later in the **Task statuses** modal.
  </Accordion>
