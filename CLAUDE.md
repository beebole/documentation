...
**AI-native endpoints (auto-generated by Mintlify):**

Mintlify automatically hosts three machine-readable endpoints at the site root, with zero maintenance: `/llms.txt` and `/llms-full.txt` (page directory for LLM indexing — relevant to our SEO/GEO work; descriptions come from page frontmatter and truncate at 300 chars / first line break) and `/skill.md` (an agentskills.io capability summary auto-generated from the docs). To override the generated `skill.md`, add `SKILL.md` files under `.mintlify/skills/<name>/` (or symlink `.mintlify/skills` to an existing folder) — note these are **published** product skills, distinct from our internal `.claude/skills/` authoring commands.

**Reverse-proxy requirement:** this site sits behind a reverse proxy configured in `website-next/next.config.ts` (sibling repo), which rewrites `beebole.com/help/*` to `beebole.mintlify.app`. Note these root files are hosted at the Mintlify app **root**, not under `/help/` (e.g. `/help/llms.txt` rewrites to `beebole.mintlify.app/llms.txt`). Status: `/help/llms.txt` and `/help/llms-full.txt` are **already forwarded**; `/help/skill.md` and the `/.well-known/skills/*` discovery endpoints are **not yet** — add explicit rewrites there if/when we adopt `skill.md`.
...
**Editorial preference (not a Mintlify limitation):**

- Mermaid diagrams (` ```mermaid `) are natively supported by Mintlify, but we avoid them — prefer `<CardGroup>`/`<Card>` with tables or other Mintlify components for a more consistent look.
