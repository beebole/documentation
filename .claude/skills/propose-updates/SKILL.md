...
description: "Map tracked app changes to documentation pages, audit them against source code, and produce a prioritized report of proposed updates. Use when asked what docs need updating, which pages are outdated, to map app changes to docs, or to prioritize documentation work."
---
...
Before mapping changes, read these context files:
...
2. **Feature area inference** — Consult the mapping table in `.claude/context/page-mappings.md` which maps keywords to likely documentation pages.
...
- **Handle `gh` failures gracefully.** If the GitHub API is unavailable, skip source code verification (step 3c) and mark affected proposals as "needs verification" rather than blocking the entire run.
