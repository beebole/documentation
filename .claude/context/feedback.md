...
Accumulated editorial rules, filed directly when the user gives feedback in conversation. Skills load this file and read the section relevant to what they're working on:
...
**Filing a new rule** (do this whenever the user gives editorial feedback that generalizes beyond one fix):

- If it applies to every page, file it under `## Site-wide`. If it applies to one page only, file it under `## Per-page` as a bullet inside an `### <URL path>` H3 (create the H3 on first entry; remove it when the last bullet leaves).
- Format: a bold one-line rule, then **Why:** (the incident or reasoning behind it) and **How to apply:** (the concrete check to run) sub-bullets.
...
- If a note is just wrong content with no rule that generalizes, fix the page directly. It does not belong here.
...
- **Don't invent features from SaaS clichés.** If a feature, label, settings path, plan tier, or workflow isn't in `../reboot`, it doesn't exist. Don't borrow what "most SaaS apps have." This includes planned-but-unshipped features: document only what's in the code today. Past leaks: "Pro and Enterprise plans", `Settings > Audit trail`, "Request changes" approval action, password-based sign-in (Beebole has **no passwords** — sign-in is a one-time 6-digit code emailed to the user, plus passkeys, Google/Microsoft, SSO), calendar views in Gantt/Kanban, diamond milestone markers — none of these are in code. (Google/Microsoft Calendar integration was on this list but **shipped in 2026**: it lives in the timesheet's external-calendar pane — `frontend/src/components/integrations/calendar/` — NOT under Settings > Integrations; it is per-user, read-only, click/drag events onto timesheet rows.)
  ...
  - **Controls suppressed on production hosts must never be documented, and re-adding one is a regression.** A control wrapped in `isProduction()` (often marked `/* SWEEP-UNDER-THE-RUG */`) exists in the code but no customer can reach it on app.beebole.com. As of 2026-09-07 this covers: the absence type's **Is paid (included in people costs)** checkbox (`absence-type-unit.ts`), the **Budget threshold alerts** notification row (`notification.ts`, also defaults to **None**), the **Push notifications** channel checkbox (`notification.ts`), the report **Schedule** / receive-by-email action (`reports/index.ts`), the **GDPR** settings entry (`settings-menu.ts`), task recurrence creation (`SHOW_TASK_RECURRENCE = false`), and the absence-quota and expense-type panels flagged in `features.md`.
    - **Why:** These have each been "corrected" back INTO the docs by a later reviewer who found the label in `labels.json` and assumed the docs were wrong. The absence **Is paid** switch was removed from `costs.mdx`/`timeoff.mdx` in August 2026 and re-added by a review agent in September 2026; budget alerts were documented as configurable for months. Every time, the result is a page telling users to click something that is not on their screen.
    - **How to apply:** Before "fixing" a page to describe a control, grep the rendering component for `isProduction()`. If the control is behind that gate, the page is right and your finding is wrong — leave it out, and check `features.md` for the entry's `_(status: hidden-flag)_` suffix.
...
- **Route navigation through the right place: sidebar vs Account Settings.** Beebole has two top-level navigation surfaces and they hold different things. Don't mix them up.
  - **Sidebar** (left nav, per `../reboot/frontend/src/app/side-bar.ts`; displayed values from the `sideBar` labels, re-verified 2026-09-07): **People**, **Projects**, **Tags** (auth-gated), plus **Planning**, **Timesheet**, **Reports**, **Journal** (always). Write *"click **X** in the sidebar"*. The planning item's label key is `sideBar.tasks` but it renders as **Planning** — never write "click **Tasks** in the sidebar". The `sideBar` group also carries **Assistant** (the AI page) and **Master Data Review**; note that one renders title-case in the sidebar group while the Settings menu entry is **Master data review**, so copy whichever surface you are describing.
  - **Settings menu** (per `../reboot/frontend/src/components/settings/settings-menu.ts`; displayed values from `settingsMenu` labels, re-verified 2026-09-07): opened from the button with the user's initials at the bottom of the sidebar. Items verbatim, in render order: **Account Settings**, **Subscription**, **Person Roles**, **Work Schedules**, **Integrations**, **Master data review**, **Time Off**, **Expense Types**, **Custom Fields**, **Release Notes**, **Delete Account**. Write *"go to **Settings** > **X**"* — and never "Settings > Account": the org-settings page is the **Account Settings** menu item.
    - The labels are **plural**: **Custom Fields** and **Expense Types**. An earlier version of this rule claimed they were singular; that was wrong and caused pages to bold labels the user cannot find. Grep `settingsMenu` in `labels.json` rather than trusting this list.
    - **GDPR** is rendered only off production (`isProduction()` gate), so it is not a route a reader can follow — don't document it. **Export your data** no longer exists as a menu item; drop it from any navigation path.
    - **Single Sign-On** has a `settingsMenu` label but is not rendered in this menu — it is reached from the Account Settings panel, per the entity-attribute case below.
...
- **Examples should match the sample data a new account contains.** Prefer the seeded names over invented ones.
  - **Why:** A new account is seeded (see `../reboot/scripts/seed-demo.mjs` / `seed-mini.mjs`) with the project categories **Client** (levels **Project** > **Subproject**), **Internal**, and **Activity**, the tag category **Department** (**Division** > **Team**), the planning **Main plan**, and projects like **Acme Corp**, **Website Redesign**, **Development** — examples that match what the reader actually sees on screen.
  - **Read the source, not the build:** the authoritative file is `../reboot/shared/i18n/config.json`. A stale copy at `../reboot/backend/dist/shared/i18n/en/config.json` still carries the old plural names (**Clients**, **Activities**) and the old **Main planning**; it is a build artifact and must not be used to "correct" a page. Re-verified 2026-09-07.
