...
- `time-tracking/calendar-view` **Timesheet calendar view** — Day-by-hour calendar alongside the timesheet grid: drag and resize entries to set start and end times directly (the duration shows while you drag), duplicate an entry by holding ⌘ while dragging it, drag a pinned favorite onto the calendar to log it instantly, click an empty slot to insert an entry with the editor opening right away, and delete from a hover button; the calendar is the default timesheet view, and hovering an entry on today's calendar reveals a play/pause button that starts or pauses a timer on it, with the duration ticking up live in place
- `time-tracking/timer` **Timer** — Start/stop timer for real-time tracking, started from the timesheet, from a calendar entry, from a pinned favorite chip, or from a suggestion (accepting it and keeping the clock running in one step)
...
- `approval/force-edit` **Force-edit a team member's timesheet** — Admins, managers, and approvers can open and edit a team member's timesheet directly from the Approval and Team panes without rejecting it, via a per-person override toggle; who may edit at each approval stage still applies (locked once approved, or when the organization restricts editing to owners and admins)
...
- `tasks/gantt` **Gantt chart view** — Timeline visualization of tasks with start/end dates, dependency links, and configurable time precision; the timeline scrolls into the past as well as forward, can be pinned to a fixed window of 1, 2, 3, 4, or 6 weeks and paged one period at a time, and planned figures can switch between hours and days
...
- `tasks/staffing` **Staffing view** — Long-horizon timeline of who works on what and when, alongside Gantt and Kanban: bars grouped by people or by project, drag on the timeline to create a booking, move or resize it, set the allocation percentage, and filter people/projects by tag; bars can be linked with the same dependencies as the Gantt (linked bookings move together), and an Unassigned row holds planned work nobody has picked up yet; the timeline can be pinned to a fixed window of 1, 2, 3, 4, or 6 weeks and paged one period at a time, bookings for tasks with hours are drawn at their real position inside the day column with non-working stretches hatched behind them, several bookings can be selected and deleted in one undoable batch, and "Copy previous period" looks back to the last period that actually has bookings and fills every empty period in between
...
- `org/configuration-export` **Master data review** — Settings screen where an administrator picks an entity (people, projects, tasks, tags, time off types, expense types, custom fields, roles, work schedules), builds columns by walking into its fields and relations, filters, includes archived records, narrows to a period, and saves as many personal reviews as wanted; each cell shows where an inherited value comes from with a link to its source, entities are clickable badges, the table re-runs live, any review downloads as JSON, CSV, TSV, Excel, or PDF, and a review can be built or refined in plain language and switched to update mode to describe a bulk change with a before/after preview and a single undo. _(status: hidden-flag — the **Master data review** Settings entry is suppressed on production hosts (`isProduction()` in `settings-menu.ts`); the route exists but is unreachable in the UI. Do NOT document until it ships. Flagged 2026-08-31)_
