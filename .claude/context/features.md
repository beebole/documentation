...
- `org/delete-protection` **Delete protection with named blockers** — Deleting an item still referenced elsewhere (a time off or expense type used by allowances or expense records, a report folder holding reports, a custom field with values, a person or project referenced by time settings, task status, schedule assignments, managers, task owners, or task projects) is refused with a readable message that names the blocking item and links straight to the panel where the reference can be removed
